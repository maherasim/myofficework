...
class Bookable extends BaseModel
{
    ...
    public function update_service_rate($reviewTo = null)
    {
        $guestReviewData = $this->reviewClass::selectRaw("AVG(rate_number) as rate_total, COUNT(*) as rate_count")
            ->where('object_id', $this->id)
            ->where('object_model', $this->type)
            ->where("status", "approved")
            ->where("review_by", "guest")
            ->first();
        $rate_number = number_format($guestReviewData->rate_total ?? 0, 1);
        $this->review_score = $rate_number;
        $this->review_count = $guestReviewData->rate_count ?? 0; 
        $this->save();

        if ($reviewTo != null) {
            $user = User::where('id', $reviewTo)->first();
            if ($user != null) {
                $reviewData = $this->reviewClass::selectRaw("AVG(rate_number) as rate_total, COUNT(*) as rate_count")
                    ->where("status", "approved")
                    ->where("review_to", $reviewTo)
                    ->first();
                $rate_number = number_format($reviewData->rate_total ?? 0, 1);
                $user->review_score = $rate_number;
                $user->review_count = $reviewData->rate_count ?? 0;
                $user->save();
            }
