...
function getSettingItem($item, $default = '', $isArray = false)
{
    $val = $default;

    $res = Settings::where('name', $item)->first();
    if ($res != null) {
        $val = $res->val;
        if ($isArray and !is_array($val)) {
...
function saveSettingItem($settingKey, $settingVal)
{
    $check = Settings::where('name', $settingKey)->first();
    if (!$check) {
        $check = new Settings();
        $check->name = $settingKey;
        $check->group = 'general';
    }
    $check->val = $settingVal;
